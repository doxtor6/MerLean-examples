...
% \ker is already defined by LaTeX
\DeclareMathOperator{\Hom}{Hom}

\title{MerLean Example: Fault-Tolerant QC}
...
\begin{abstract}
This document presents the mathematical content from the \texttt{QEC1} library, formalized from the paper \href{https://arxiv.org/abs/2410.02213}{2410.02213}.
...
\begin{proof}
Let $L'$ be any logical operator of the deformed code that achieves the minimum weight $d^* = |L'|$. We will show that $|L'| \geq \min(h(G), 1) \cdot d$.
...
Let $G$ be a graph satisfying the exactness condition with Cheeger constant $h(G) > 0$. Let $d$ denote the original code distance and $d^*$ the deformed code distance. Then:
\begin{enumerate}
\item \textbf{When $h(G) \geq 1$}: $d^* \geq d$ (distance is preserved).
\item \textbf{When $h(G) < 1$}: $d^* \geq h(G) \cdot d$ (distance is reduced by factor $h(G)$).
\item \textbf{In all cases}: $d^* \geq \min(h(G), 1) \cdot d$.
\end{enumerate}

Furthermore, $h(G) = 1$ is optimal in the sense that:
\begin{itemize}
\item No improvement occurs for $h(G) > 1$ (the bound remains $d^* \geq d$)
\item Distance degradation occurs for $h(G) < 1$ (the bound becomes $d^* \geq h(G) \cdot d < d$)
\item When $h(G) = 1$ and a trivially extendable logical exists, $d^* = d$ exactly
\end{itemize}
\end{corollary}
...
\textbf{Part 1} ($h(G) \geq 1 \implies d^* \geq d$): When $h(G) \geq 1$, we have $\min(h(G), 1) = 1$. Applying the space distance bound gives $d^* \geq 1 \cdot d = d$.

\textbf{Part 2} ($h(G) < 1 \implies d^* \geq h(G) \cdot d$): When $h(G) < 1$, we have $\min(h(G), 1) = h(G)$. The space distance bound then gives $d^* \geq h(G) \cdot d$. Since $h(G) < 1$ and $d > 0$, we obtain $h(G) \cdot d < d$, confirming distance reduction.
...
\subsubsection*{Introduction to Flux Check Measurement Frequency}
...
\subsubsection*{The Gauging Approach to Logical Operator Measurement}
